module mac_128x128(
    ...
    assign o_addr_in= (state==S_IDLE)?7'd0:i_data_wd[6:0];
    ...
    assign o_data_tmp=r_accum +i_data_tmp; //항상 tmp에서 읽어서 더해서 저장
    ...
    always@(*)begin
        ...
        //나머지 경우는 addr_row 유지
        //ctrl_addr_row==1'b1이면 addr_row 유지, 0이면 addr_row 증가
        if(state!=S_FETCH_PTR && o_addr_row==7'd0&&r_last_row==1'b1) begin
            ctrl_addr_row=1'b1;
        end
        else if(next_state==S_FETCH_PTR || next_state==S_WAIT0) begin
            ctrl_addr_row=1'b0;
        end
        ...
        // wait1이면 10
        // ctrl_cnt==00이면 r_cnt-1, 01이면 i_data_row, 10이면 r_cnt 유지
        ...
        //나머지 01
        // ctrl_addr_data==00이면 addr_data+1, 01이면 addr_data 유지, 10이면 addr_data 초기화
        if(next_state==S_DATA && r_addr_data!=14'd0) begin
            ctrl_addr_data=(r_addr_data==i_nnz-1)?2'b10:2'b00;
        end
        else if(next_state==S_FETCH_PTR) begin
            ctrl_addr_data=2'b00;
        end
        else if(r_addr_data==14'd0) begin
            ctrl_addr_data=2'b01;
        end
        ...
        //write 될 때 1
        //r_wait==1 일때 1
        // ctrl_addr_bias==1이면 addr_bias 증가, 0이면 addr_bias 유지
        if(o_addr_bias==7'd127) begin
            ctrl_addr_bias=1'b0;
        end
        else if(r_write_n==1'b1) begin
            ctrl_addr_bias=1'b0;
        end
        else if(o_we_tmp==1'b1) begin
            ctrl_addr_bias=1'b1;
        end
        else if(o_addr_bias==7'd0 && r_addr_data==14'd0) begin
            ctrl_addr_bias=1'b0;
        end
        ...
        else if(o_addr_bias==7'd0) begin
            ctrl_addr_bias=1'b0;
        end
        else begin
            ctrl_addr_bias=1'b0;
    ...
    always@(posedge i_clk or negedge i_rst_n) begin
        if(!i_rst_n) begin
            ...
            else if (o_addr_row==7'd127)begin
                r_last_row<=1;
            end
            else begin
                o_done<=o_done;
