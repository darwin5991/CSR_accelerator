module PE_128x512 (
    ...
    wire [2:0] idx;
    assign idx= (i_layer_num*4) + ctrl_cnt; // layer_num에 따라 4개의 quarter 중 하나 선택
    ...
    always@(posedge i_clk or negedge i_rst_n) begin
        if(!i_rst_n) begin
            ...
            o_done_delay<=2'b00;
            r_addr_base <= 16'd0;
            r_addr_base_row <= 9'd128*i_pe_num; // pe_num에 따라 row_base 주소 초기값 설정 (0, 128, 256, 384)
        end
            ...
            if(ctrl_cnt!=2'd3)begin
                r_addr_base <= r_addr_base + r_nnz[idx];
                r_addr_base_row <= r_addr_base_row + 9'd128;
            end
            else begin
                // r_addr_base <= (933+907+891+897)*i_layer_num;
                r_addr_base <= (887+845+806+707)*i_layer_num;
